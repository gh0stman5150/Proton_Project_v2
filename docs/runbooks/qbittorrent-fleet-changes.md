...
If runtime verification fails before the change, stop and diagnose. A rolling structural deployment must not begin on top of an unhealthy, zombie, or `D`-state member.
...
2. refuses before the first change if any instance is absent, unhealthy, zombie, or has a `D`-state task;
3. reads each instance's own live Proton state;
4. forces the common synchronizer to recreate one instance at a time;
...
- [ ] Pre-existing unhealthy/zombie/`D`-state member resolved before rollout.
