...
Proton NAT-PMP response
  ...
  └─ invoke proton-qbittorrent-sync-safe.sh <instance>
       ...
       ├─ inspect container for manual stop, zombie, and D state
...
On a failed recreation, the script attempts to restore the previous published artifact and service port. It must never substitute another instance's port.
...
edit shared source once
  ...
  -> refuse if any member is unhealthy/zombie/D-state
...
Shared preferences should instead be managed as a declared key list. During the 2026-08-14 audit, these unexplained differences were found and must be reviewed during the post-reboot maintenance window:
...
If any task is in uninterruptible `D` state, especially with `folio_wait_bit_common` and a netfs/CIFS kernel trace, the host kernel is the recovery boundary. Automation must not keep issuing Compose, Docker remove, cgroup kill, shim kill, or signal operations.
...
The reconciler preflights the entire fleet before changing the first member, refuses unhealthy/zombie/`D`-state baselines, recreates sequentially, and stops at the first failed health gate.
