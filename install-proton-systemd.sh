...
validate_bundle() {
	...
	ensure_source_file "${SCRIPT_DIR}/tools/proton-fleet-services.sh"
...
install_qbittorrent_fleet_verifier() {
	...
	install_normalized_file \
		"${SCRIPT_DIR}/tools/proton-fleet-services.sh" \
		"${BIN_DIR}/proton-fleet-services.sh" 0755
}
...
log "Templated instance units installed; running instance chains have not been upgraded by file copying alone"
log "Follow docs/runbooks/qbittorrent-fleet-changes.md for approved sequential activation of lidarr, prowlarr, radarr, sonarr, and whisparr"
log "Legacy two-field leases need a new tunnel generation and fresh UDP/TCP lease before updated sync; include watchers and finish with fleet runtime verification"
